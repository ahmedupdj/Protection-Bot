...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('antikick') 
        .setDescription('Anti-kick protection settings')
        .addStringOption(option =>
            option
                .setName('mode')
                .setDescription('Set the mode to enable/disable kick protection')
                .setRequired(true)
                .addChoices(
                    { name: 'on', value: 'true' },
        ...
        .addIntegerOption(option =>
            option
                .setName('limit')
                .setDescription('Set the limit for kicking')
                .setRequired(true)
        )
        .addIntegerOption(option =>
            option
                .setName('punishment')
                .setDescription('Set the punishment for violating the kick limit')
                .setRequired(true)
                .addChoices(
    ...
    async execute(interaction, client) {
        ...
        if (!settings) {
            settings = new ProtectionSettings({
                ...
                isCodeEnabledAntiKicks: mode === 'true' ? true : false,
                LimitKicks: limit || 3, 
                PunishmentKicks: punishment || 1
            });
        } else {
            settings.isCodeEnabledAntiKicks = mode === 'true' ? true : false;
            settings.LimitKicks = limit || 3; 
            settings.PunishmentKicks = punishment || 1; 
        }
        ...
        const exampleEmbed = new EmbedBuilder()
            .setColor(0x0099FF)
            .setTitle('Anti Kick Settings')
            .setDescription(`Anti Kick settings updated successfully`)
            .addFields({name:'Kick Limit', value: `${settings.LimitKicks}`})
